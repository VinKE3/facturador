...
<Invoice xmlns="urn:oasis:names:specification:ubl:schema:xsd:Invoice-2"
    ...
    @php($expected_amount = round($discount->factor * $discount->base, 2))
    @php($show_multiplier_factor_numeric = $expected_amount - 1 < $discount->amount && $discount->amount < $expected_amount + 1)
    <cac:AllowanceCharge>
        <cbc:ChargeIndicator>false</cbc:ChargeIndicator>
        <cbc:AllowanceChargeReasonCode>{{ $discount->discount_type_id }}</cbc:AllowanceChargeReasonCode>
        @if($show_multiplier_factor_numeric)
