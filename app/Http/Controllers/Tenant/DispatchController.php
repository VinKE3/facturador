...
use App\Imports\DispatchesDataMassiveImport;
...
//            });
    ...
    {
        return view('tenant.dispatches.massive');
    }

    public function ReadDataMassive(Request $request)
    {
        if ($request->hasFile(('file'))) {
            $import = new DispatchesDataMassiveImport();
            $import->import($request->file('file'), null, Excel::XLSX);
            $data = $import->getData();

            return [
                'success' => true,
                'message' => 'Guías de remisión cargadas correctamente.',
                'data' => $data
            ];
        }
