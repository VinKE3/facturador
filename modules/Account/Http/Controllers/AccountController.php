...
use Modules\Account\Exports\ReportAccountingUtilconExport;
...
class AccountController extends Controller
{
    ...
    public function download(Request $request)
    {
        ...
        switch ($type) {
            ...
            case 'utilcon':
                $data = [
                    'records' => $this->getStructureUtilcon($this->getAllDocuments($d_start, $d_end)),
                ];

                $filename = 'Provision-Ventas_'.date('YmdHis');

                return (new ReportAccountingUtilconExport)
                    ->data($data)
                    ->download($filename.'.xlsx');
        }
    }
    ...
    private function getStructureUtilcon($documents)
    {
        $rows = [];

        foreach ($documents as $index => $row){
            if ($row->state_type_id ==  '09' || $row->state_type_id == '11') {
                $rows[] = [
                    'item' => $index + 1,
                    'fecha' => $row->date_of_issue->format('d/m/Y'),
                    'vencimiento' => '',
                    'tipo' => $row->document_type_id,
                    'numero' => $row->series.'-'.$row->number,
                    'moneda' => $row->currency_type_id === 'PEN' ?'S/.':'US$',
                    'cliente_numero_documento_identidad' => '0001',
                    'cliente_nombre' => 'ANULADO',
                    'valor' => 0,
                    'venta_bolsa' => '',
                    'exonerado' => 0,
                    'igv' => 0,
                ...
                $rows[] = [
                    'item' => $index + 1,
                    'fecha' => $row->date_of_issue->format('d/m/Y'),
                    'vencimiento' => isset($row->invoice) ? $row->invoice->date_of_due->format('d/m/Y') : '',
                    'tipo' => $row->document_type_id,
                    'numero' => $row->series.'-'.$row->number,
                    'moneda' => $row->currency_type_id === 'PEN' ? 'S/.':'US$',
                    'cliente_numero_documento_identidad' => $row->customer->number,
                    'cliente_nombre' => $row->customer->name,
                    'valor' => $row->total_taxed + $row->total_exportation + $row->total_unaffected,
                    'venta_bolsa' => '',
                    'exonerado' => $row->total_exonerated === 0 ? '' : $row->total_exonerated,
                    'igv' => $row->total_igv,
                    'icbper' => $row->total_plastic_bag_taxes,
                    'percepcion' => '',
                    'total' => $row->total,
                    'sub' => '05',
                    'costo' => '',
                    'ctacble' => '701111',
                    'glosa' => '',
                    'tdoc_ref' => isset($row->note) ? $row->note->affected_document->document_type_id : '',
                    'numero_ref' => isset($row->note) ? $row->note->affected_document->series.'-'.$row->note->affected_document->number: '',
                    'fecha_ref' => isset($row->note) ? $row->note->affected_document->date_of_issue->format('d/m/Y') : '',
                    'igv_ref' =>  isset($row->note) ? $row->note->affected_document->total_igv : '',
                    'base_imp_ref' =>  isset($row->note) ? $row->note->affected_document->total_taxed + $row->note->affected_document->total_exportation + $row->note->affected_document->total_unaffected + $row->note->affected_document->total_exonerated : '',
                ];   
            }
        }
